    ...
    <div className="container mx-auto px-4 py-12">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-bold mb-4">Crea Tu Plan Personalizado</h2>
          ...
          <CardContent className="p-8">
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="space-y-3">
                <Label className="text-lg font-semibold">¿Cuál es tu edad?</Label>
                <Input
                  type="number"
                  min="13"
                  max="100"
              ...
              {/* Nivel de Pasos */}
              <div className="space-y-3">
                <Label className="text-lg font-semibold">¿Cuántos pasos das diariamente?</Label>
                <RadioGroup
                  value={formData.stepsLevel}
                  onValueChange={(value) => setFormData(prev => ({ ...prev, stepsLevel: value as UserData['stepsLevel'] }))}
                  className="space-y-3"
                >
                  <div className="flex items-center space-x-3 bg-muted p-4 rounded-lg cursor-pointer hover:bg-opacity-80">
                    <RadioGroupItem value="basic" id="basic" data-testid="radio-steps-basic" />
                    <Label htmlFor="basic" className="cursor-pointer flex-1">
                      <div className="font-medium">Menos de 4,000 pasos - Nivel Básico</div>
                      <div className="text-sm text-muted-foreground">Estilo de vida sedentario, poco movimiento diario</div>
                    </Label>
                  </div>
                  <div className="flex items-center space-x-3 bg-muted p-4 rounded-lg cursor-pointer hover:bg-opacity-80">
                    <RadioGroupItem value="medium" id="medium" data-testid="radio-steps-medium" />
                    <Label htmlFor="medium" className="cursor-pointer flex-1">
                      <div className="font-medium">4,000 - 6,000 pasos - Nivel Medio</div>
                      <div className="text-sm text-muted-foreground">Actividad moderada, algunas caminatas</div>
                    </Label>
                  </div>
                  <div className="flex items-center space-x-3 bg-muted p-4 rounded-lg cursor-pointer hover:bg-opacity-80">
                    <RadioGroupItem value="advanced" id="advanced" data-testid="radio-steps-advanced" />
                    <Label htmlFor="advanced" className="cursor-pointer flex-1">
                      <div className="font-medium">Más de 6,000 pasos - Nivel Avanzado</div>
                      <div className="text-sm text-muted-foreground">Persona activa, ejercicio regular</div>
                    </Label>
                  </div>
                </RadioGroup>
              </div>
              ...
              {/* Estado de Salud */}
              <div className="space-y-3">
                <Label className="text-lg font-semibold">¿Cómo describes tu estado de salud?</Label>
                <Select onValueChange={(value) => setFormData(prev => ({ ...prev, healthStatus: value as UserData['healthStatus'] }))}>
                  <SelectTrigger className="bg-muted border-border text-white" data-testid="select-health-status">
              ...
              <div className="space-y-3">
                ...
                <Select onValueChange={(value) => setFormData(prev => ({ ...prev, mainObjective: value as UserData['mainObjective'] }))}>
                  <SelectTrigger className="bg-muted border-border text-white" data-testid="select-main-objective">
              ...
              <Button 
                type="submit" 
                className="w-full bg-primary hover:bg-primary/80 text-primary-foreground font-bold py-4 px-6 text-lg"
